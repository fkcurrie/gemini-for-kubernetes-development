...
func (r *Reconciler) reconcileReviews(ctx context.Context, repoWatch *reviewv1alpha1.RepoWatch, ghClient *github.Client, owner string, repo string, user *github.User, podsBySandbox map[string]*corev1.Pod) error {
	...
	watchedPRs, pendingPRs, activeSandboxes := r.reconcileReviewSandboxesInternal(ctx, user, repoWatch, ghClient, explicitPRs, prs, sandboxList, podsBySandbox)
...
func (r *Reconciler) reconcileReviewSandboxesInternal(ctx context.Context, user *github.User, repoWatch *reviewv1alpha1.RepoWatch, ghClient *github.Client, explicitPRs []*github.PullRequest, prs []*github.PullRequest, sandboxes *unstructured.UnstructuredList, podsBySandbox map[string]*corev1.Pod) ([]reviewv1alpha1.WatchedPR, []int, int) {
	log := log.FromContext(ctx)
	...
	for _, pr := range allPRs {
		...
		if sandboxExists {
			...
			if prIsExplicit || (activeSandboxes < repoWatch.Spec.Review.MaxActiveSandboxes) &&
				...
				if err := r.createReviewSandboxForPR(ctx, user, ghClient, repoWatch, pr); err != nil {
...
// sandbox.
func (r *Reconciler) createReviewSandboxForPR(ctx context.Context, user *github.User, ghClient *github.Client, repoWatch *reviewv1alpha1.RepoWatch, pr *github.PullRequest) error {
	log := log.FromContext(ctx)
	sandboxName := fmt.Sprintf("%s-pr-%d", repoWatch.Name, *pr.Number)
...
func (r *Reconciler) reconcileDevSandboxes(ctx context.Context, user *github.User, repoWatch *reviewv1alpha1.RepoWatch, ghClient *github.Client, upstreamRepo string, podsBySandbox map[string]*corev1.Pod) error {
	...
	watchedDevSandboxes, pendingDevBranches, err := r.reconcileDevSandboxesInternal(ctx, user, ghClient, repoWatch, branches, forkOwner, forkRepo, podsBySandbox)
	if err != nil {
		return err
	}
...
func (r *Reconciler) reconcileDevSandboxesInternal(ctx context.Context, user *github.User, ghClient *github.Client, repoWatch *reviewv1alpha1.RepoWatch, branches []*github.Branch, forkOwner, forkRepo string, podsBySandbox map[string]*corev1.Pod) ([]reviewv1alpha1.DevSandbox, []string, error) {
	log := log.FromContext(ctx)
