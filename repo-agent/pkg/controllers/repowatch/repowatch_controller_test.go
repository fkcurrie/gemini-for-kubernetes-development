...
func TestReconcileReviewSandboxes_RespectsExistingActiveSandboxes(t *testing.T) {
	...
	// Call reconcile
	watchedPRs, pendingPRs, activeSandboxes := r.reconcileReviewSandboxesInternal(context.Background(), &github.User{Login: github.String("test-user")}, repoWatch, nil, []*github.PullRequest{}, openPRs, existingSandboxList, map[string]*corev1.Pod{})
